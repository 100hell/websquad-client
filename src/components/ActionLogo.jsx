import {
  Box,
  Button,
  Drawer,
  DrawerBody,
  DrawerContent,
  DrawerHeader,
  DrawerOverlay,
  Flex,
  Input,
  Skeleton,
  Stack,
  Text,
  useColorModeValue,
  useDisclosure,
} from "@chakra-ui/react";
import React, { useEffect, useState } from "react";
import { useShowToast } from "../hooks/useShowToast";
import { useRecoilState, useRecoilValue } from "recoil";
import userAtom from "../atom/userAtom";
import Comment from "./Comment";
import { BsSend } from "react-icons/bs";
import { ChevronDownIcon, ChevronUpIcon } from "@chakra-ui/icons";
import postAtom from "../atom/postAtom";
import UserFollower from "./UserFollower";

const ActionLogo = ({ post, postPage }) => {
  const user = useRecoilValue(userAtom);
  const isPostLiked = post?.likes.includes(user?._id);
  const [liked, setLiked] = useState(isPostLiked);
  const [reply, setReply] = useState("");
  const [isreplying, setIsReplying] = useState(false);
  const [posts, setPosts] = useRecoilState(postAtom);
  const [userLiked, setUserLiked] = useState([]);

  const [isLikingpost, setIsLikingPost] = useState(false);

  const showToast = useShowToast();
  const { isOpen, onOpen, onClose } = useDisclosure();
  const {
    isOpen: isLikeOpen,
    onOpen: onLikeOpen,
    onClose: onLikeClose,
  } = useDisclosure();

  const handleLikePost = async (e) => {
    e.preventDefault();
    if (isLikingpost) return;
    setIsLikingPost(true);
    if (!user) {
      showToast(
        "Error",
        "You must be logged in to Like/Unlike the post",
        "error"
      );
      return;
    }
    try {
      const res = await fetch(`/api/posts/like/${post._id}`, {
        method: "PUT",
        headers: {
          "Content-type": "application/json",
        },
      });
      const data = await res.json();
      if (data.error) {
        showToast("Error", data.error, "error");
        return;
      }
      console.log(data);
      if (!liked) {
        const updatedPost = posts.map((p) => {
          if (p._id == post._id) {
            return { ...p, likes: [...p.likes, user._id] };
          }
          return p;
        });
        setPosts(updatedPost);
      } else {
        const updatedPost = posts.map((p) => {
          if (p._id == post._id) {
            return { ...p, likes: p.likes.filter((id) => id !== user._id) };
          }
          return p;
        });
        setPosts(updatedPost);
      }
    } catch (error) {
      showToast("Error", error, "error");
    } finally {
      setIsLikingPost(false);
    }
    setLiked(!liked);
  };

  const handleReplyPost = async (e) => {
    e.preventDefault();
    if (!user) {
      showToast("Error", "You must be logged in to reply to the post", "error");
      return;
    }
    setIsReplying(true);
    try {
      const res = await fetch(`/api/posts/reply/${post._id}`, {
        method: "PUT",
        headers: {
          "Content-type": "application/json",
        },
        body: JSON.stringify({ text: reply }),
      });
      const data = await res.json();
      if (data.error) {
        showToast("Error", data.error, "error");
        return;
      }
      onClose();
      setReply("");
      showToast("Success", "Reply sent succesfully", "success");
      const updatedPost = posts.map((p) => {
        if (p._id == post._id) {
          return { ...p, replies: data };
        }
        return p;
      });
      setPosts(updatedPost);
    } catch (error) {
      showToast("Error", error, "error");
    } finally {
      setIsReplying(false);
    }
  };

  const handleViewPostLikes = (e) => {
    setUserLiked([]);
    e.preventDefault();
    if (!user) {
      showToast(
        "Error",
        "You must be logged in to Like/Unlike the post",
        "error"
      );
      return;
    }
    onLikeOpen();
    const getPostLikeprofile = async (user) => {
      try {
        const res = await fetch(`/api/users/profile/${user}`);
        const data = await res.json();
        if (data.error) {
          showToast("Error", data.error, "error");
          return;
        }
        setUserLiked((userLiked) => [...userLiked, data]);
      } catch (error) {
        showToast("Error", error, "error");
      }
    };

    post.likes.map((user) => {
      getPostLikeprofile(user);
    });
  };
  return (
    <Flex flexDirection={"column"} gap={1}>
      <Flex
        gap={3}
        my={2}
        onClick={(e) => {
          e.preventDefault();
        }}
      >
        {/*------------------------- LIKE--------------------------*/}
        <svg
          aria-label="Like"
          color={liked ? "rgb(237, 73, 86)" : ""}
          fill={liked ? "rgb(237, 73, 86)" : "transparent"}
          height="19"
          role="img"
          viewBox="0 0 24 22"
          width="20"
          onClick={handleLikePost}
        >
          <path
            d="M1 7.66c0 4.575 3.899 9.086 9.987 12.934.338.203.74.406 1.013.406.283 0 .686-.203 1.013-.406C19.1 16.746 23 12.234 23 7.66 23 3.736 20.245 1 16.672 1 14.603 1 12.98 1.94 12 3.352 11.042 1.952 9.408 1 7.328 1 3.766 1 1 3.736 1 7.66Z"
            stroke="currentColor"
            strokeWidth="2"
          ></path>
        </svg>
        {/* -----------------------End Like------------------------------*/}

        {/* ----------------------Comment ----------------------------- */}
        {!postPage && (
          <svg
            aria-label="Comment"
            color=""
            fill=""
            height="20"
            role="img"
            viewBox="0 0 24 24"
            width="20"
            onClick={onOpen}
          >
            <title>Comment</title>
            <path
              d="M20.656 17.008a9.993 9.993 0 1 0-3.59 3.615L22 22Z"
              fill="none"
              stroke="currentColor"
              strokeLinejoin="round"
              strokeWidth="2"
            ></path>
          </svg>
        )}
        {/* -----------------------End Comment--------------------------------- */}

        {/* ----------------------------REPOST ------------------------------- */}
        <RepostSVG />
        {/* ------------------------End Repost-------------------------------- */}

        {/* -------------------------- SHARE -------------------------------------- */}
        <ShareSVG />
        {/* ------------------------End Share-------------------------------------*/}
      </Flex>
      <Flex gap={2} alignItems={"center"}>
        <Text
          color={"gray.light"}
          fontSize={"sm"}
          onClick={handleViewPostLikes}
          cursor={"pointer"}
        >
          {post.likes.length} likes
        </Text>
        <Box
          w={".5"}
          h={".5"}
          borderRadius={"full"}
          background={"gray.light"}
        ></Box>
        <Text color={"gray.light"} fontSize={"sm"}>
          {post.replies.length} replies
        </Text>
      </Flex>

      {/* Likes Drawer  */}

      <Drawer placement={"top"} onClose={onLikeClose} isOpen={isLikeOpen}>
        <DrawerOverlay />
        <DrawerContent
          w={"620px"}
          maxW={"620px"}
          bg={useColorModeValue("gray.300", "gray.dark")}
          margin={"0 auto"}
          minH={"100vh"}
        >
          <DrawerHeader borderBottomWidth="1px">
            <Flex justifyContent={"space-between"} w={"100%"}>
              {" "}
              <span>{post.likes.length} likes</span>{" "}
              <ChevronUpIcon onClick={onLikeClose} />
            </Flex>{" "}
          </DrawerHeader>
          <DrawerBody padding={"0px"}>
            {userLiked &&
              userLiked.map((userlike) => {
                return (
                  <UserFollower
                    key={userlike._id}
                    user={userlike}
                    CloseDrawer={onLikeClose}
                  />
                );
              })}
            {userLiked.length === 0 && (
              <Flex textAlign={"center"} justifyContent={"center"}>
                No Likes Found
              </Flex>
            )}
          </DrawerBody>
        </DrawerContent>
      </Drawer>
      {/* -------------------------- */}
      {/* Reply modal */}
      <Flex maxW={"620px"}>
        <Drawer
          placement={"bottom"}
          onClose={onClose}
          isOpen={isOpen}
          size={"xs"}
        >
          <DrawerOverlay />
          <DrawerContent
            bg={useColorModeValue("gray.300", "gray.dark")}
            maxW="620px"
            w="620px"
            margin={"0 auto"}
            borderRadius="12px 12px 0 0 "
          >
            <DrawerHeader borderBottomWidth="1px">
              <Flex justifyContent={"space-between"}>
                <span>Comments</span>
                <span>
                  <ChevronDownIcon onClick={onClose} />
                </span>
              </Flex>
            </DrawerHeader>
            <DrawerBody>
              {post ? (
                post.replies.map((reply, index) => {
                  const reverseReply =
                    post.replies[post.replies.length - 1 - index];
                  return (
                    <Comment
                      key={reverseReply?._id}
                      reply={reverseReply}
                      post={post}
                    />
                  );
                })
              ) : (
                <Stack>
                  {" "}
                  <Skeleton height="20px" />
                  <Skeleton height="20px" />
                  <Skeleton height="20px" />
                </Stack>
              )}
            </DrawerBody>
            <Flex gap={3}>
              <Input
                placeholder="your reply goes here..."
                value={reply}
                onChange={(e) => setReply(e.target.value)}
              />
              <Button
                colorScheme="blue"
                mr={3}
                size={"sm"}
                onClick={handleReplyPost}
                color={useColorModeValue("black", "white")}
                bg={useColorModeValue("gray.300", "transparent")}
                isLoading={isreplying}
              >
                <BsSend size={"20px"} />
              </Button>
            </Flex>
          </DrawerContent>
        </Drawer>
      </Flex>
    </Flex>
  );
};

export default ActionLogo;

const RepostSVG = () => {
  return (
    <svg
      aria-label="Repost"
      color="currentColor"
      fill="currentColor"
      height="20"
      role="img"
      viewBox="0 0 24 24"
      width="20"
    >
      <title>Repost</title>
      <path
        fill=""
        d="M19.998 9.497a1 1 0 0 0-1 1v4.228a3.274 3.274 0 0 1-3.27 3.27h-5.313l1.791-1.787a1 1 0 0 0-1.412-1.416L7.29 18.287a1.004 1.004 0 0 0-.294.707v.001c0 .023.012.042.013.065a.923.923 0 0 0 .281.643l3.502 3.504a1 1 0 0 0 1.414-1.414l-1.797-1.798h5.318a5.276 5.276 0 0 0 5.27-5.27v-4.228a1 1 0 0 0-1-1Zm-6.41-3.496-1.795 1.795a1 1 0 1 0 1.414 1.414l3.5-3.5a1.003 1.003 0 0 0 0-1.417l-3.5-3.5a1 1 0 0 0-1.414 1.414l1.794 1.794H8.27A5.277 5.277 0 0 0 3 9.271V13.5a1 1 0 0 0 2 0V9.271a3.275 3.275 0 0 1 3.271-3.27Z"
      ></path>
    </svg>
  );
};

const ShareSVG = () => {
  return (
    <svg
      aria-label="Share"
      color=""
      fill="rgb(243, 245, 247)"
      height="20"
      role="img"
      viewBox="0 0 24 24"
      width="20"
    >
      <title>Share</title>
      <line
        fill="none"
        stroke="currentColor"
        strokeLinejoin="round"
        strokeWidth="2"
        x1="22"
        x2="9.218"
        y1="3"
        y2="10.083"
      ></line>
      <polygon
        fill="none"
        points="11.698 20.334 22 3.001 2 3.001 9.218 10.084 11.698 20.334"
        stroke="currentColor"
        strokeLinejoin="round"
        strokeWidth="2"
      ></polygon>
    </svg>
  );
};
